import { navItems, services } from "@/helpers/helpers";
import { Car, Instagram, MapPin, Phone } from "lucide-react";
import Image from "next/image";
import Link from "next/link";
import React from "react";

function Footer() {
  const currentYear = new Date().getUTCFullYear();
  return (
    <footer className="bg-gray-900 text-gray-300 py-16">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          <div>
            <div className="flex items-center space-x-3 mb-6">
              {/* <div className="bg-linear-to-br from-blue-600 to-blue-800 p-3 rounded-xl">
                <Car className="w-8 h-8 text-white" />
              </div> */}
              <Image
                src={"/logo.png"}
                width={60}
                height={60}
                alt="Logo"
                className="rounded-md"
              />
              <div>
                <div className="font-bold text-xl text-white">Miraç Oto</div>
                <div className="text-sm text-blue-400">Özel Servis</div>
              </div>
            </div>
            <p className="text-gray-400 leading-relaxed">
              VAG grubu araçlarınız için güvenilir ve profesyonel servis
              hizmeti.
            </p>
          </div>

          <div>
            <h3 className="text-white font-bold text-lg mb-4">Hızlı Linkler</h3>
            <ul className="space-y-3">
              {navItems.map((item) => (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className="hover:text-blue-400 transition"
                  >
                    {item.title}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          <div>
            <h3 className="text-white font-bold text-lg mb-4">Hizmetler</h3>
            <ul className="space-y-3">
              {services(null, null, null, null, null, null).map((item) => (
                <li key={item.title}>
                  <Link
                    href="/hizmetler"
                    className="hover:text-blue-400 transition"
                  >
                    {item.title}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          <div>
            <h3 className="text-white font-bold text-lg mb-4">İletişim</h3>
            <ul className="space-y-3">
              <li className="flex items-start">
                <MapPin className="w-9 h-9 mr-2 mt-1 text-blue-400" />
                <address className="not-italic text-white leading-relaxed">
                  Hacı İshak Mahallesi,
                  <br />
                  Nuri Giyik Sanayi Sitesi, 31. Blok, Küme Evler No: 12/7,
                  <br />
                  45200 Akhisar / Manisa, Türkiye
                </address>
              </li>
              <li className="flex items-start">
                <Phone className="w-5 h-5 mr-2 mt-1 text-blue-400" />
                <div>
                  <a
                    href="tel:+905359898350"
                    className="block hover:text-blue-400 transition"
                  >
                    0535 989 8350
                  </a>
                </div>
              </li>
              <li className="flex items-start">
                <Phone className="w-5 h-5 mr-2 mt-1 text-blue-400" />
                <div>
                  <a
                    href="tel:+905072964042"
                    className="block hover:text-blue-400 transition"
                  >
                    0507 296 4042
                  </a>
                </div>
              </li>
              <li className="flex items-start">
                <Phone className="w-5 h-5 mr-2 mt-1 text-blue-400" />
                <div>
                  <a
                    href="tel:+905053994619"
                    className="block hover:text-blue-400 transition"
                  >
                    0505 399 4619
                  </a>
                </div>
              </li>
              <li className="flex items-start">
                <Instagram className="w-5 h-5 mr-2 mt-1 text-blue-400" />
                <a
                  href="https://www.instagram.com/miracozelservis/"
                  target="_blank"
                  className="hover:text-blue-400 transition"
                >
                  @miracozelservis
                </a>
              </li>
            </ul>
          </div>
        </div>

        <div className="border-t border-gray-800 pt-8">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <p className="text-gray-400 mb-4 md:mb-0">
              © {currentYear} Miraç Oto Özel Servis. Tüm hakları saklıdır.
            </p>
            <div className="flex space-x-6">
              <Link
                href="gizlilik-politikasi"
                className="text-gray-400 hover:text-blue-400 transition"
              >
                Gizlilik Politikası
              </Link>
              <Link
                href="kullanim-sartlari"
                className="text-gray-400 hover:text-blue-400 transition"
              >
                Kullanım Şartları
              </Link>
            </div>
          </div>
        </div>
      </div>
      <a
        href="https://wa.me/905359898350"
        target="_blank"
        rel="noopener noreferrer"
        className="fixed right-6 bottom-6 flex items-center justify-center w-14 h-14 bg-green-500 text-white rounded-full shadow-lg hover:bg-green-600 hover:scale-110 transition-transform duration-300 z-50"
        aria-label="WhatsApp ile iletişime geçin"
      >
        <svg
          role="img"
          viewBox="0 0 24 24"
          xmlns="http://www.w3.org/2000/svg"
          className="w-7 h-7 fill-current"
        >
          <title>WhatsApp</title>
          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
        </svg>
      </a>
    </footer>
  );
}

export default Footer;
